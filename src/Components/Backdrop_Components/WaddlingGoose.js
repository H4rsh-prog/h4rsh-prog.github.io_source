...
export default function WaddlingGoose() {
    ...
    useEffect(()=>{
        ...
        const waddle_distance = Math.floor(window.outerWidth+600) - 80;
        const waddle_duration = 9*(waddle_distance/1400);
        gsap.timeline({repeat:-1}).fromTo("#waddling-goose", {x: waddle_distance}, {x: browser.DPI == "MOBILE" ? "-120" : "0", duration: waddle_duration, ease: "none"})
